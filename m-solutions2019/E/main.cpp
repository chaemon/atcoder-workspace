...
template<int mod=MOD>
inline Num<mod> __mod_pow(const Num<mod> &x, Int k){
	if(k==0) return 1;
	Num<mod> res(__mod_pow(x,k/2));
	res*=res;
	if(k%2)res*=x;
	return res;
}
template<class T, int mod=MOD>
inline Num<mod> mod_pow(T x, Int k){
	return mod_pow(Num<mod>(x),k);
}
//}}}
...
void solve(){
	REP(q,Q){
		if(d[q]==0){
			cout<<mod_pow(x[q],n[q])<<endl;
		}else{
			int t = int(mod_int(x[q])/d[q]);
			if(t + n[q] - 1 >= MOD){
				cout<<0<<endl;
			}else{
				cout<<fact(t+n[q]-1)/fact(t-1)*mod_pow(mod_int(d[q]),n[q])<<endl;
			}
		}
	}
}
