...
proc main() =
#  p := rect(A.float, H.float * (30.0/360.0) * (PI * 2.0))
#  q := rect(B.float, M.float * (6.0/360.0) * (PI * 2.0))
  x := (H.float + M.float/60.0) * (30.0/360.0) * (PI * 2.0)
  y := M.float * (6.0/360.0) * (PI * 2.0)
  p := complex(A.float * cos(x), A.float * sin(x))
  q := complex(B.float * cos(y), B.float * sin(y))
