...
proc main() =
  desk := newSeqWith(N, initDoublyLinkedList[int]())
  container := newSeq[DoublyLinkedNode[int]]()
  for i in 0..<N:
    container.add(newDoublyLinkedNode(i))
    desk[i].append(container[i])
  for q in 0..<Q:
#    echo "query: ", q
#    dump(f[q] + 1)
#    dump(t[q] + 1)
...
#      echo ""
#    echo " ================ "
    var tmp = desk[f[q]].split(container[x[q]])
    desk[t[q]].merge(tmp)
