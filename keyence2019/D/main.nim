...
proc main() =
  cmb := initCombination[Mint]()
  v := newSeq[(int,int)]()
  for i in 0..<N:
    v.add((A[i], 0))
  for i in 0..<M:
    v.add((B[i], 1))
  if A.toSet().len != N: echo 0;return
  if B.toSet().len != M: echo 0;return
  v.sort()
  v.reverse()
  if v[0][0] != N * M or v[1][0] != N * M: echo 0;return
  if v[2][0] != N * M - 1: echo 0;return
  var
    i = 2
    R = 1
    C = 1
  ...
  while i < v.len:
    var j :int
    if i + 1 < v.len and v[i][0] == v[i + 1][0]:
      j = i + 2
      var next = if j == v.len: 0 else: v[j][0]
      let d = v[i][0] - next - 1
      rest += R + C
      if i > 0: ans *= cmb.P(rest, d)
      rest -= d
      R += 1
      C += 1
    else:
