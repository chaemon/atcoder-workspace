...
if a < 0 or b < 0 or (not((a mod 3 == 0) and (b mod 3 == 0))):
  echo 0
  quit()
...
proc modfac(x: int ): int =
  result = 1
  for i in (2..x):
    result = (result * i) mod MOD

proc modbinom(x, y: int ): int =
  var p = modfac(x)
  let mfy = modfac(y)
  var (s, t) = extgcd(mfy, MOD)
  s = ((s mod MOD) + MOD) mod MOD
  p *= s
  p = p mod MOD
  var (u, v) = extgcd(modfac(x-y), MOD)
  u = ((u mod MOD) + MOD) mod MOD
  p *= u
  p = p mod MOD
  return p
#  return  modfac(x) * extgcd(modfac(y), MOD)[0] mod MOD  * extgcd(modfac(x-y), MOD)[0] mod MOD


echo modbinom(a+b, min(a, b))
