...
proc solve(N:int, x:seq[float], y:seq[float]) =
  var v = newSeq[float]()
  for i in 0..<N:
    if x[i] == 0 and y[i] == 0: continue
    let p = Complex((x[i],y[i]))
    let (r,phi) = polar(p)
    v.add(phi-PI/2.0)
    v.add(phi+PI/2.0)
    v.add(phi)
  ...
  for theta in v:
    ...
    var
      ...
      sy = 0.0
    for i in 0..<N:
      if x0 * x[i] + y0 * y[i] >= 0.0:
        sx += x[i];sy += y[i]
    ans.max= sqrt(sx*sx+sy*sy)
...
proc main() =
  var N = 0
  N = nextInt()
  var x = newSeqWith(N, 0.0)
  var y = newSeqWith(N, 0.0)
  for i in 0..<N:
    x[i] = nextFloat()
    y[i] = nextFloat()
  solve(N, x, y);
