...
proc test():void = 
  mp := initOrderedMap[int, OrderedMap[int,string]]()
  echo "nya"
#  mp[12] = initOrderedMap[int,string]()
  mp[12][13] = "twenty"
#  mp[19] = initOrderedMap[int,string]()
  mp[19][17] = "nineteen"
  echo mp[12][13]
  echo mp
...
#  test()
#  return
  s := initOrderedMultiSet[int]()
  rates := newSeqWith(M, initOrderedMultiSet[int]())
  v := newSeq[Node[int,int]](N)
  w := newSeq[Node[int,int]](M)
  ...
  for q in 0..<Q:
    i := C[q]
    rates[B[i]].remove(v[i])

    s.remove(w[B[i]])
    if rates[B[i]].len > 0:
      h := - rates[B[i]].front.key
      w[B[i]] = s.insert(h)
    else:
