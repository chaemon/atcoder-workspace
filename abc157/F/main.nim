...
proc f(t:float):bool =
  ...
  for i in 0..<N:
    r := t / c[i]
    v.add(initCircle(p[i], r))
  x := newSeq[Point]()
  for i in 0..<N:
    x.add(v[i].p)
    for j in i+1..<N:
      if not intersect(v[i], v[j]) == 0: continue
      var q = crosspoint(v[i], v[j])
      x.add(q[0])
      x.add(q[1])
  for p in x:
    var ans = 0
...
#  let mx = 300000.0
  assert(f(mx))
  echo findFirst(f, 0.0, mx, 1e-9).formatfloat(ffDecimal, 10)
  return
