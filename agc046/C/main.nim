...
proc propagate[T](self: var CumulativeSum[T]) =
  while self.data.len < self.pos + 2:
    self.data.setLen(self.pos + 2)
...
proc main() =
  ...
  v.add(ones)
  K.min=S.len
  dp := Array(B, B, Mint(0)) # receive, # of move
  dp[0][0] = Mint(1)
  for t in v:
    var dp2 = Array(B, B, Mint(0))
    # x <= t (stay: x, move: t - x, receive: 0): dp2[r][m] += dp[r + (t - x)][m - (t - x)]
    # x > t (stay: t, move: 0, receive: x - t): dp2[r][m] += dp[r - (x - t)][m]
    block:
      var css: array[B + B - 1, CumulativeSum[Mint]]
      for i in 0..<css.len:
        css[i] = initCumulativeSum[Mint]()
      for r in 0..<B:
        for m in 0..<B:
          css[r + m][r] = dp[r][m]
      for r in 0..<B:
        for m in 0..<B:
          dp2[r][m] += css[r + m][r..r+t]
    block:
      var css: array[B, CumulativeSum[Mint]]
      for i in 0..<css.len:
        css[i] = initCumulativeSum[Mint]()
      for r in 0..<B:
        for m in 0..<B:
