...
#  g.graphviz()
  ...
  if r == -1:
    echo 1;return
  proc dfs(g:Graph[int], u:int, p:int):(bool, int) =
    if g[u].len == 1: return (true, 1)
    var l = if r != u and g[u].len == 2: true else: false
    var s = 0
    var found = false
    var ct = 0
    for e in g[u]:
      if e.dst == p: continue
      ct.inc
      let (l2, n) = g.dfs(e.dst, u)
      s += n
      if l2: found = true
      else: l = false
    if ct == 1 and found: return (true, 1)
    if found: s.dec
    return (false, s)
