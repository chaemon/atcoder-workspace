...
struct P{
	Int a;
	int u;
	P(Int a,int u):a(a),u(u){}
	bool operator<(const P &p)const{//REVERSE!!
		return a > p.a;
	}
};
...
void solve(){
	Graph G(N);
	REP(i,M)addBiEdge(G,{A[i],B[i]},C[i]);
	auto a = shortest_path(G,0);
	auto b = shortest_path(G,N-1);
	vector<vector<Int> > dist(K+1,vector<Int>(N,inf<Int>()));
	dist[0][0] = 0;
	for(int k = 0;k <= K;k++){
		static vector<bool> vis;
		vis.assign(N,false);
		priority_queue<P> q;
		REP(u,N)q.push(P(dist[k][u],u));
		for(;!q.empty();){
			auto t = q.top();q.pop();
			int u = t.u;
			if(vis[u])continue;
			...
			int k2 = min(k+X[u],K);
			Int a2 = a + Y[u];
			dist[k2][u] = min(dist[k2][u], a2);
			for(auto &&e:G[u]){
				if(dist[k][e.dst] > a + e.weight){
					dist[k][e.dst] = a + e.weight;
					q.push(P(a + e.weight,e.dst));
				}
			}
		}
	}
	if(dist[K][N-1]==inf<Int>()){
		cout<<-1<<endl;
	}else{
		cout<<dist[K][N-1]<<endl;
	}
}
