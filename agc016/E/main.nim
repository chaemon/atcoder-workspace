...
proc main() =
  ans := 0
  var reserve = newSeqWith(N, newSeqWith(N, false))
  var live = newSeqWith(N, true)
  for u in 0..<N:
    reserve[u][u] = true
    for i in countdown(M - 1, 0):
      if reserve[u][x[i]] and reserve[u][y[i]]:
        live[u] = false
        break
      if reserve[u][x[i]] or reserve[u][y[i]]:
        reserve[u][x[i]] = true
        reserve[u][y[i]] = true
  for u in 0..<N:
    if not live[u]: continue
    for v in u+1..<N:
      if not live[v]: continue
      found := false
      for w in 0..<N:
        if reserve[u][w] and reserve[v][w]: found = true
      if not found: ans.inc
  print ans
  return
