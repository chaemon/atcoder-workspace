...
proc solve() =
  ans := int.inf
  g := initGraph[int](N)
  for i in 0..<M: g.addBiEdge(u[i], v[i], l[i])
  for i in 0..<g[0].len:
    let t = g[0][i].dst
    g2 := initGraph[int](N)
    for i in 0..<M:
      if u[i] == 0 and v[i] == t: continue
      if u[i] == t and v[i] == 0: continue
      g2.addBiEdge(u[i], v[i], l[i])
    let (dist, _) = g2.dijkstra(0)
    if dist[t] != int.inf:
      ans.min= g[0][i].weight + dist[t]
  if ans == int.inf:
    echo -1
  else:
    echo ans
