...
proc decode(p:int):seq[int] =
  var p = p
  result = newSeq[int](N)
  for i in 0..<N:
    result[i] = p mod (A[i] + 1)
    p = p div (A[i] + 1)

proc encode(v:seq[int]):int =
  result = 0
  var p = 1
  for i in 0..<N:
    result += v[i] * p
...
#  echo prod
  country := newSeq[int](A.sum)
  ct := 0
  for i in 0..<N:
    for j in 0..<A[i]:
      country[ct] = i
      ct += 1
  if not (N <= 10 and A.max <= 3): doAssert(false)
#  a := newSeq[int](A.sum)
#  for i in 0..<A.sum:
#    a[i] = i
#  var ans = 0
...
#      ci := country[a[i]]
#      ci2 := country[a[i + 1]]
#      if abs(ci - ci2) <= 1: valid = false
#    if valid: ans += 1
#    if not a.nextPermutation(): break
#  echo ans
  P := 1
  for i in 0..<N: P *= A[i] + 1
  var dp = newSeqWith(P, N, initMint(0))
  dp[0][0] = initMint(1)
  for p in 0..<P:
    v := p.decode
    for j in 0..<N:
      if p == 0 and j > 0: continue
      for i in 0..<N:
        if v[i] == A[i]: continue
        if p > 0 and abs(i - j) <= 1: continue
#        dp[v.encode][i] += dp[p][j] * (A[i] - v[i] + 1)
#        doAssert(v.encode == p + prod[i])
#        v[i] += 1
        dp[p + prod[i]][i] += dp[p][j] * (A[i] - v[i])
#        v[i] -= 1
  echo dp[P-1].sum
  return
